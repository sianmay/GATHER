...
class Gather(Core.Model):
    ...
    def __init__(self, env_size: int, home_size: int, deposit_rate: float, hdecay_rate: float, fdecay_rate: float,
                 communication_network, social_network, cost: int, cost_frequency: int,  random_chance: float = 0.0,
        ...
        if move_mode == 'RMS':
            ...
            self.systems.add_system(Agents.PheromoneMovementSystem('PMS', self, communication_network, social_network,
    ...
    def create_environment_resources(self, mode: int):
        ...
        if mode == Gather.ENV_RANDOM:
            ...
            from PIL import Image
            generator = np.asarray(Image.open('./resources/1patch.png').convert('L')) / 255.0
            generator = generator.flatten()
            generator = np.where(generator > 0.0, 2, 1)
